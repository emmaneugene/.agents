 ...
 *
 * - Injects a visual overlay into the active tab,
 * - Lets the user interactively select one or more DOM elements by clicking them.
 * - A banner is shown at the bottom of the page with the provided message and instructions.
 *
 * Interactions (in the browser):
 *   Click              — Select a single element and finish immediately.
 *   Cmd/Ctrl + Click   — Add an element to the multi-selection (keeps picker open).
 *   Enter              — Confirm and return all accumulated selections.
 *   ESC                — Cancel and return null.
 *
 * For each selected element the following info is returned and printed:
 *   tag, id, class, text (first 200 chars), html (first 500 chars), parents
 *
 * Usage:
 *   browser-pick.js '<instruction message>'
 *
 * Example:
